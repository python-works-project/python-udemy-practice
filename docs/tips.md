...
このページでは、Python学習の中で出てきた補足事項やVSCodeの操作方法など、理解を深めるためのポイントを簡潔にまとめています。
...
- [VSCode 実行方法の違いまとめ](#VSCode-実行方法の違いまとめ)
- [](#)
- [](#)
- [](#)
- [](#)
...
- ジェネレータ内包表記：(式 for 変数 in イテラブル if 条件)
```
...
### VSCode 実行方法の違いまとめ
```
- ターミナル実行：launch.json 無効・デバッグ不可・環境変数は OS/シェル依存
- デバッグ実行 (F5)：launch.json 有効・デバッグ可能・環境変数は env や .env を反映
```
